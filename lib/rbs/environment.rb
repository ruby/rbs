    ...
    def insert_decl(decl, context: nil, namespace: Namespace.root)
      ...
      when AST::Declarations::Class, AST::Declarations::Module
        ...
        when decl.is_a?(AST::Declarations::Class) && existing_entry.is_a?(ClassEntry)
          ...
          raise DuplicatedDeclarationError.new(name, decl, existing_entry.each_decl.first || raise)
        end

        decl.each_decl do |d|
          insert_decl(d, context: [context, name], namespace: name.to_namespace)
        end
    ...
    def resolve_type_names(only: nil)
      ...
      each_rbs_source do |source|
        ...
        if !resolve || resolve.value
          map = UseMap.new(table: table)
          source.directives.each do |dir|
            if dir.is_a?(AST::Directives::Use)
              dir.clauses.each do |clause|
                map.build_map(clause)
              end
            end
        ...
        source = Source::RBS.new(source.buffer, source.directives, decls)
