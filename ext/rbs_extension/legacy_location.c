...
static rbs_constant_id_t rbs_constant_pool_insert_ruby_symbol(VALUE symbol) {
    ...
    uint8_t *start = malloc(RSTRING_LEN(name));
    memcpy((void *) start, RSTRING_PTR(name), RSTRING_LEN(name));

    return rbs_constant_pool_insert_owned(RBS_GLOBAL_CONSTANT_POOL, start, RSTRING_LEN(name));
}
