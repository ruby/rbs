...
EOF
    ...
    env.insert_decl(decls[0])
    env.insert_decl(decls[1])
    env.insert_decl(decls[2])
    env.insert_decl(decls[3])
...
EOF
    ...
    env.add_signature(RBS::Source::RBS.new(buf, dirs, decls))

    env_ = env.resolve_type_names

    writer = RBS::Writer.new(out: StringIO.new)

    writer.write(env_.each_decl.to_a)
